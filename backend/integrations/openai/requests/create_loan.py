...
from integrations.openai.prompts.create_loan import ANALYZE_APPROVE_AND_CREATE_LOAN
...
class LoanVerdictAIPydantic(BaseModel):
...
# print(ANALYZE_APPROVE_AND_CREATE_LOAN)


def test_loan():  # TEMP FOR TESTING ONLY!!!

    client = OpenAI()

    pdf_path = settings.BASE_DIR / 'financial_statements/cashflows/cashflow_2_domus.pdf'
    ...
    # 2. Call responses.create with file + schema
...
# '{"status":"rejected","loan_amount":0,"annual_interest_rate":0,"payment_amount":0,"term_months":12,"analysis_summary":"The 12-month cash flow shows consistent negative net cash flow each month, culminating in a total net cash flow of -1,190,000 and an ending cash balance of -1,040,000. This indicates insufficient liquidity and ongoing operating losses, representing a high credit risk with no evident near-term cash generation to support debt service. Based on these metrics, the portfolio cannot reasonably support new loan disbursement under standard terms."}'
